...
const float THRESHOLD = 100.0; // Threshold for values z_k
const int MAX_ITER = 500; // Maximum count of iterations
...
// Indicator is a float belong to [0, 1] to render color. 1 - belong to set, otherwise - don't belong
float computeIndicator() {
    Complex c = screenToWorld(gl_FragCoord.xy / resolution);
